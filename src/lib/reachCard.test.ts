...
describe('G-GA-10 — the Q&A row renders the provider state honestly', () => {
  it('unknown renders NEUTRAL — no cause, no connect copy (the R3-300 rule)', () => {
    const rows = computeReachRows({ providerState: { status: 'unknown' }, chatGranted: true, catalogAnswered: true, writable: true, sourceShared: true, mountId: null, toolsSupported: true });
  ...
  it('not-configured names the KEY cause; ungranted names the CONSENT cause — never conflated', () => {
    const noKey = computeReachRows({ providerState: { status: 'not-configured' }, chatGranted: true, catalogAnswered: true, writable: true, sourceShared: false, mountId: null, toolsSupported: true });
    expect(row(noKey, 'answer').state).toBe('blocked');
    expect(row(noKey, 'answer').cause).toContain('add one in Settings');

    const forbidden = computeReachRows({ providerState: configured(), chatGranted: false, catalogAnswered: true, writable: true, sourceShared: false, mountId: null, toolsSupported: true });
  ...
  it('the host-marked UNGRANTED state names the consent cause WITHOUT a configured provider (R3-688)', () => {
    ...
    const marked = computeReachRows({ providerState: { status: 'ungranted' }, chatGranted: false, catalogAnswered: true, writable: true, sourceShared: false, mountId: null, toolsSupported: true });
  ...
  it('the mark ALONE blocks — ungranted with a granted-looking catalog (R3-688, isolating)', () => {
    ...
    const marked = computeReachRows({ providerState: { status: 'ungranted' }, chatGranted: true, catalogAnswered: true, writable: true, sourceShared: false, mountId: null, toolsSupported: true });
    expect(row(marked, 'answer').state).toBe('blocked');
    expect(row(marked, 'answer').cause).toContain("wasn't granted chat");
    expect(row(marked, 'answer').cause).not.toContain('Settings');
  ...
  it('an UNANSWERED catalog names no cause — neutral, not a guessed consent state', () => {
    ...
    const waiting = computeReachRows({ providerState: { status: 'not-configured' }, chatGranted: false, catalogAnswered: false, writable: true, sourceShared: false, mountId: null, toolsSupported: true });
    ...
    const keyed = computeReachRows({ providerState: configured(), chatGranted: false, catalogAnswered: false, writable: true, sourceShared: false, mountId: null, toolsSupported: true });
    expect(row(keyed, 'answer').state).toBe('neutral');
    expect(reachChips(keyed).some((c) => /summarize|tagged security/i.test(c))).toBe(false);
  });
  ...
  it('the host MARK does not wait for the catalog — ungranted blocks with no answer at all', () => {
    ...
    const marked = computeReachRows({ providerState: { status: 'ungranted' }, chatGranted: false, catalogAnswered: false, writable: true, sourceShared: false, mountId: null, toolsSupported: true });
    expect(row(marked, 'answer').state).toBe('blocked');
    expect(row(marked, 'answer').cause).toContain("wasn't granted chat");
  });

  it('configured + granted is ✓ and carries the read-flavored chips', () => {
    const ok = computeReachRows({ providerState: configured(), chatGranted: true, catalogAnswered: true, writable: false, sourceShared: false, mountId: null, toolsSupported: true });
...
describe('G-GA-1 — the read/draft/apply rows derive from writability, never copy', () => {
  it('read-only blocks Draft with the reader cause and yields NO write chip', () => {
    const rows = computeReachRows({ providerState: configured(), chatGranted: true, catalogAnswered: true, writable: false, sourceShared: false, mountId: null, toolsSupported: true });
  ...
  it('writable turns Draft ✓ — and its chip is a DESCRIBE, not an apply', () => {
    const rows = computeReachRows({ providerState: configured(), chatGranted: true, catalogAnswered: true, writable: true, sourceShared: false, mountId: null, toolsSupported: true });
...
describe('R3-752 — the apply row is a destination, across every input (R-GA-3)', () => {
    ...
    for (const sourceShared of [true, false]) {
      for (const providerState of states) {
        it(`writable=${writable} sourceShared=${sourceShared} provider=${providerState.status} → elsewhere with a destination and no cause`, () => {
          const rows = computeReachRows({ providerState, chatGranted: true, catalogAnswered: true, writable, sourceShared, mountId: 'space:abc', toolsSupported: true });
  ...
  it('the source-trust sentence is a property of the SOURCE — never on the apply row', () => {
    const rows = computeReachRows({ providerState: configured(), chatGranted: true, catalogAnswered: true, writable: true, sourceShared: true, mountId: null, toolsSupported: true });
...
describe('R3-752 — the packaging row states the substrate, as context (neutral, no chips)', () => {
  it('mountId: null is the own-corpus row; an id is the mounted row', () => {
    const own = computeReachRows({ providerState: configured(), chatGranted: true, catalogAnswered: true, writable: true, sourceShared: true, mountId: null, toolsSupported: true });
    expect(row(own, 'packaging').label).toBe('Reads its own entries');
    expect(row(own, 'packaging').state).toBe('neutral');

    const mounted = computeReachRows({ providerState: configured(), chatGranted: true, catalogAnswered: true, writable: true, sourceShared: true, mountId: 'space:abc', toolsSupported: true });
  ...
  it('the packaging row contributes no chips — either packaging, same chip set', () => {
    const own = reachChips(computeReachRows({ providerState: configured(), chatGranted: true, catalogAnswered: true, writable: true, sourceShared: true, mountId: null, toolsSupported: true }));
    const mounted = reachChips(computeReachRows({ providerState: configured(), chatGranted: true, catalogAnswered: true, writable: true, sourceShared: true, mountId: 'space:abc', toolsSupported: true }));
          ...
          for (const mountId of [null, 'space:abc']) {
            for (const toolsSupported of [true, false]) {
              for (const chatGranted of [true, false]) {
                const rows = computeReachRows({ providerState, chatGranted, catalogAnswered: true, writable, sourceShared, mountId, toolsSupported });
...
describe('R3-752 / G-GA-8 — the tool-less degrade is a visible qualifier, never a silent downgrade', () => {
  it('configured + granted + tools=false keeps row 1 ✓ and adds the degrade qualifier', () => {
    const degraded = computeReachRows({ providerState: configured(false), chatGranted: true, catalogAnswered: true, writable: true, sourceShared: true, mountId: null, toolsSupported: false });
  ...
  it('tools=true restores the plain label; a blocked row never grows the qualifier', () => {
    const fine = computeReachRows({ providerState: configured(true), chatGranted: true, catalogAnswered: true, writable: true, sourceShared: true, mountId: null, toolsSupported: true });
    expect(row(fine, 'answer').label).toBe('Answer questions about this wiki');

    const noKey = computeReachRows({ providerState: { status: 'not-configured' }, chatGranted: true, catalogAnswered: true, writable: true, sourceShared: true, mountId: null, toolsSupported: false });
...
describe('G-GA-6 — envelope flips recompute rows and chips together', () => {
  it('disconnecting the provider flips answer ✓→blocked and drops its chips', () => {
    const before = computeReachRows({ providerState: configured(), chatGranted: true, catalogAnswered: true, writable: true, sourceShared: false, mountId: null, toolsSupported: true });
    const after = computeReachRows({ providerState: { status: 'not-configured' }, chatGranted: true, catalogAnswered: true, writable: true, sourceShared: false, mountId: null, toolsSupported: true });
  ...
  it('revoking write flips draft ✓→blocked on the next computation', () => {
    const before = computeReachRows({ providerState: configured(), chatGranted: true, catalogAnswered: true, writable: true, sourceShared: false, mountId: null, toolsSupported: true });
    const after = computeReachRows({ providerState: configured(), chatGranted: true, catalogAnswered: true, writable: false, sourceShared: false, mountId: null, toolsSupported: true });
